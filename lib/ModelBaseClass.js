...
class ModelBaseClass {
	...
	instantiateProperty(type, value, createByClassName) {
		...
		switch (type) {
			...
			case 'datetime':
				// Special cases for YYYYMMDD and YYYY-MM-DD
				if (value.length === 8) {
				...
				if (value.length === 10) {
					...
					const day = parseInt(value.substring(8));
					return new Date(year, month - 1, day);
				}
			...
			case 'float':
				...
				const candidateFloat = parseFloat(String(value).trim());
				return Number.isFinite(candidateFloat) ? candidateFloat : null;

			case 'integer':
				if (typeof value === 'number') return Number.isFinite(value) ? Math.trunc(value) : null;
				if (value === '' || value === null) return null;
				const candidateInteger = parseInt(String(value).trim(), 10);
				return Number.isNaN(candidateInteger) ? null : candidateInteger;
